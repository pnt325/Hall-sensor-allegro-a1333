...
//#include "board.h"
        ...
        //digitalWrite(cs, LOW);
#define GPIO_LOW(pin)     digitalWrite(pin, LOW)
#define GPIO_HIGH(pin)    digitalWrite(pin, HIGH)
...
uint16_t A1333::readEEPROM(uint16_t addr, uint32_t& value)
{
    return ExtendedRead(10, addr, value);
}
...
uint16_t A1333::writeEEPROM(uint16_t addr, uint32_t value)
{
  return ExtendedWrite(10, addr, value);
}
...
boolean A1333::begin(int csPin)
{
  ...
  while ((flags & 0x0001) != 0x0001)
  {
    ...
    if (t0==0)
    {
      Serial.println("Error: Device is not Unlocked");
      error=true;
      a1333 = false;
      return false;
    }
